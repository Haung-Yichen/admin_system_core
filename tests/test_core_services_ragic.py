    ...
    def test_field_config_initialization(self):
        ...
        config = {
            "ragic": {
                "field_email": "1000381",
                "field_name": "1000376",
                "field_door_access_id": "1000375"
            }
        }
        
        field_config = RagicFieldConfig(config)
        
        assert field_config.email_id == "1000381"
        assert field_config.name_id == "1000376"
        assert field_config.door_access_id == "1000375"
...
class TestRagicService:
    ...
    def test_get_field_value_by_id(self, ragic_service):
        ...
        record = {
            "1000381": "test@example.com",
            "1000376": "Test User"
        }
        
        value = ragic_service._get_field_value(record, "1000381")
    ...
    async def test_verify_email_exists_found(self, ragic_service):
        ...
        with patch.object(ragic_service, 'get_all_employees', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = [
                {"_ragic_id": "1", "1000381": "test@example.com", "1000376": "Test User"}
            ]
    ...
    async def test_verify_email_exists_not_found(self, ragic_service):
        ...
        with patch.object(ragic_service, 'get_all_employees', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = [
                {"_ragic_id": "1", "1000381": "other@example.com", "1000376": "Other User"}
            ]
    ...
    def test_parse_employee_record_fallback_to_ragic_id(self, ragic_service):
        """Test _parse_employee_record() uses ragic_id if no door access ID."""
        record = {
            "_ragic_id": "123",
            "1000381": "test@example.com",
            "1000376": "Test User"
