...
class TestEventLoopNonBlocking:
    ...
    async def test_health_check_not_blocked_by_vector_search(self):
        ...
        print(
            f"[PASS] Health check completed in {health_check_times[0]*1000:.2f}ms while search was running")
    ...
    async def test_multiple_concurrent_lightweight_requests(self):
        ...
        tasks = [heavy_task()] + [light_request(i)
                                  for i in range(NUM_LIGHT_REQUESTS)]
        ...
        print(
            f"[PASS] {NUM_LIGHT_REQUESTS} light requests: avg={avg_time*1000:.2f}ms, max={max_time*1000:.2f}ms")
...
class TestDatabaseConnectionPool:
    ...
    async def test_pool_configuration_increased(self):
        ...
        with patch('core.app_context.ConfigLoader') as MockLoader:
            ...
            with patch('core.database.engine.create_async_engine') as mock_create:
                ...
                print(
                    f"[PASS] Pool configured: pool_size=20, max_overflow=40 (max 60 connections)")
    ...
    async def test_concurrent_db_sessions_simulation(self):
        ...
        print(
            f"[PASS] {NUM_CONCURRENT} concurrent operations: avg={avg_time*1000:.2f}ms, all successful")
...
class TestVectorServiceThreadSafety:
    ...
    async def test_concurrent_embedding_generation(self):
        ...
        print(
            f"[PASS] {NUM_CONCURRENT} concurrent embeddings generated without interference")
    ...
    async def test_asyncio_to_thread_preserves_event_loop_responsiveness(self):
        ...
        print(
            f"[PASS] Event loop remained responsive ({check_count} checks passed)")
...
class TestBackgroundThreadSafety:
    ...
    async def test_no_attached_to_different_loop_error(self):
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            bg_futures = [executor.submit(
                background_thread_work) for _ in range(3)]
...
class TestSearchServiceIntegration:
    ...
    async def test_search_with_mocked_dependencies(self):
        ...
        elapsed = time.perf_counter() - start
        print(
            f"[PASS] Search service structure verified (took {elapsed*1000:.2f}ms)")
...
class TestStressSimulation:
    ...
    async def test_100_concurrent_requests_simulation(self):
        ...
        print(
            f"[PASS] 100 concurrent requests completed in {total_time*1000:.1f}ms")
        print(
            f"       Light requests (n={NUM_LIGHT}): avg={avg_light*1000:.2f}ms")
        print(
            f"       Heavy requests (n={NUM_HEAVY}): avg={avg_heavy*1000:.2f}ms")
