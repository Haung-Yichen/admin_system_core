...
class FastAPIServer:
    ...
    def _register_routes(self, app: FastAPI) -> None:
        ...
        async def line_webhook_dynamic(
            ...
            if module is None:
                self._logger.warning(f"LINE webhook: Module '{module_name}' not found")
                raise HTTPException(
            ...
            if line_config is None:
                self._logger.warning(f"Module '{module_name}' does not support LINE webhook")
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
            ...
            if not channel_secret:
                self._logger.error(f"Module '{module_name}' has no LINE channel secret configured")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            if not self._verify_line_signature(body, x_line_signature, channel_secret):
                self._logger.warning(f"Invalid LINE signature for module '{module_name}'")
                self._context.log_event(f"LINE webhook: Invalid signature for {module_name}", "SECURITY")
                raise HTTPException(
            ...
            for event in events:
                try:
                    ...
                    self._logger.error(f"Error handling LINE event in '{module_name}': {e}")
        ...
        async def line_webhook(request: Request) -> Dict[str, Any]:
            ...
            try:
                payload = await request.json()
                self._context.log_event("Received LINE webhook (deprecated endpoint)", "WEBHOOK")
